...
export const parseCSV = (csvText: string, fs: number = 1600): RawDataPoint[] => {
  ...
  const cleanHeader = (h: string) => {
    // Remove quotes, whitespace, and BOM
  ...
  let axIdx = headers.indexOf('ax');
  let ayIdx = headers.indexOf('ay');
  let azIdx = headers.indexOf('az');

  // Fallback: Check if they are part of a string like "Time, Accel X, Accel Y..."
  if (axIdx === -1) axIdx = headers.findIndex(h => h.includes('ax') || h.includes('x-axis') || h.includes('acc x'));
  if (ayIdx === -1) ayIdx = headers.findIndex(h => h.includes('ay') || h.includes('y-axis') || h.includes('acc y'));
  if (azIdx === -1) azIdx = headers.findIndex(h => h.includes('az') || h.includes('z-axis') || h.includes('acc z'));

  if (axIdx === -1 || ayIdx === -1 || azIdx === -1) {
    throw new Error("CSV must contain 'ax', 'ay', and 'az' columns (or similar). Found: " + headers.join(', '));
  }
...
// Updated: Uses Hanning Window + RMS Correction
export const calculateFFT = (data: number[], fs: number): FFTResult[] => {
  const n = data.length;
  if (n === 0) return [];
  
  // Power of 2 truncation
  ...
  // --- AMPLITUDE CORRECTION FACTOR (ACF) vs ENERGY CORRECTION FACTOR (ECF) ---
  const windowCorrection = 2.0;
  
  // Convert Peak to RMS (Root Mean Square)
  // RMS = Peak * 0.7071
  const rmsCorrection = 0.7071;

  for (let i = 0; i < N / 2; i++) {
    const freq = i * fs / N;
    
    // Ignore DC (0Hz) and everything above Nyquist limit
    // The range is now dynamic based on sample rate (fs)
    if (freq >= 0.5 && freq <= fs / 2) { 
      // 1. Basic FFT Norm
      let magnitude = Math.sqrt(real[i] * real[i] + imag[i] * imag[i]) / N;
      
      // 2. Single-Sided Spectrum Correction (* 2)
      if (i > 0) magnitude *= 2; 

      // 3. Window Correction (ACF=2.0)
      magnitude *= windowCorrection;

      // 4. Convert to RMS (Effective Value)
      magnitude *= rmsCorrection;
